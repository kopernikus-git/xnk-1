...
PrivacyWidget::PrivacyWidget(EncoCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zXNK anonymizes your XNK by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zXNK or convert back to XNK"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 XNK ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zXNK");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zXNK");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zXNK");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zXNK");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zXNK");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zXNK");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zXNK");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zXNK");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zXNK Movements"));
    ...
    // Only Convert to XNK enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzXNK->setTitleClassAndText("btn-title-grey", "Total 0 zXNK");
    ui->btnTotalzXNK->setSubTitleClassAndText("text-subtitle", "Show denominations of zXNK owned.");
    ui->btnTotalzXNK->setRightIconClass("ic-arrow");
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zXNK");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of XNK to mint into zXNK"));
    }else{
        btnText = tr("Convert back to XNK");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zXNK to convert back into XNK"));
    }
...
void PrivacyWidget::onTotalZxnkClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzXNK->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzXNK->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zXNK is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zXNK, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zXNK").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zXNK minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZxnk(
        ...
        // Spend succeed
        inform(tr("zXNK converted back to XNK"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzXNK->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
